...
- **Nav links** (docs-home in the sidebar, Geoscape Hub in the topbar): both
  default to the values below; override either in `conf.py`
  ```python
  html_context = {
      "docs_home_url": "https://docs.geoscape.com.au",
      "docs_home_label": "Geoscape Documentation",
      "hub_url": "https://hub.geoscape.com.au/",
      "hub_label": "Go to Hub",
  }
  ```
...
|-----------|---------|-------|
| `pyproject.toml` → `version` | `1.0.5` | **Exact**, no `v` prefix (PEP 440). Never `v1`. |
| `__init__.py` → `__version__` | `1.0.5` | Same value as pyproject. |
| Exact git tag | `v1.0.5` | `v` prefix; points at this commit. |
| Moving major tag | `v1` | `v` prefix; **moved** to the same commit. |

**One version number** (`1.0.5`) goes in the two Python files; **two git tags**
(`v1.0.5` and `v1`) point at the same commit. The moving `v1` tag is only a git
...
# 2. Bump BOTH files to the new exact version (e.g. 1.0.5):
#    - pyproject.toml     version = "1.0.5"
#    - __init__.py        __version__ = "1.0.5"
#    - add a CHANGELOG.md entry

# 3. Commit, tag exact + move the major tag, push:
git commit -am "Release 1.0.5: <what changed>"
git tag v1.0.5            # exact tag == pyproject version, with a leading v
git tag -f v1             # move the major alias onto the same commit
git push origin master v1.0.5
...
rebuild. Repos pinned to an exact tag (`@v1.0.5`) adopt it by re-pinning
